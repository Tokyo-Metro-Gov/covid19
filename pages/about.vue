<template>
  <div class="About">
    ...
    <StaticCard>
      {{
        $t(
          '当サイトは新型コロナウイルス感染症 (COVID-19) に関する最新情報を提供するために、Code for OKINAWAが開設したものです。'
        )
      }}<br />
      {{
        $t(
          '複製・改変が許されたオープンソースライセンスで公開されている、 東京都公式コロナウイルス対策サイトの仕組みを利用しています。'
        )
      }}<br />
      <br />
      {{
        $t(
          '沖縄県による公式情報と客観的な数値をわかりやすく伝えることで、沖縄県にお住まいの方や、沖縄県内に拠点を持つ企業の方、沖縄県を訪れる方が、現状を把握して適切な対策を取れるようにすることを目的としています。'
        )
      }}
    </StaticCard>
    <StaticCard>
    ...
    <StaticCard>
      ...
      >
        <template v-slot:catalogWebsite>
          <a
            href="https://www.pref.okinawa.jp/site/hoken/chiikihoken/kekkaku/covid19_hasseijoukyou.html"
            target="_blank"
            rel="noopener noreferrer"
          >
            {{ $t('沖縄県のサイト') }}
